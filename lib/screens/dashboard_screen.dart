...
import 'dart:convert';
import 'login_screen.dart'; // Importiamo il Login per il Logout!
...
class _DashboardScreenState extends State<DashboardScreen> {
    ...
    if (listaFiltrata.isEmpty) {
      return RefreshIndicator(
        ...
        child: ListView(
          ...
          children: const [
            SizedBox(height: 200),
            Center(child: Text("Nessun film trovato.", style: TextStyle(fontSize: 18))),
          ],
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    final filmDaVedere = _film.where((f) => f['visto'] == null || f['visto'] == false).toList();
    final filmVisti = _film.where((f) => f['visto'] != null && f['visto'] != false).toList();
